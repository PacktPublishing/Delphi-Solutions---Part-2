...
type
  TwmMain = class(TWebModule)
    ...
    procedure WebModule1DefaultHandlerAction(Sender: TObject; Request: TWebRequest;
      ...
      var Handled: Boolean);
    procedure wmMainwaPhotoAction(Sender: TObject; Request: TWebRequest; Response: TWebResponse;
      var Handled: Boolean);
...
procedure TwmMain.WebModule1DefaultHandlerAction(Sender: TObject; Request: TWebRequest;
Response: TWebResponse;
var Handled: Boolean);
var
...
begin
  ...
  try
    ...
    lHTMLOut.AppendLine('  body {font-family: Verdana; padding: 40px 10px 0px 50px; }');
    ...
    begin
      ...
      lFileName := 'images' + PathDelim + FormatDateTime(DATEFORMAT, lFileTimeStamp) + '.png';
      if TFile.Exists(lFileName) then
      begin
        lHTMLOut.AppendFormat('<h3>Last update %s</h3>', [DateTimeToStr(lFileTimeStamp)]);
        ...
        begin
          try
            ...
            lJSONInfo := TJSONObject.ParseJSONValue(lJSONInfoString) as TJSONObject;
            ...
            begin
              ...
              lHTMLOut.AppendFormat('<pre>Lat: %3.8f Lon: %3.8f</pre>', [lLat, lLon]);
            end
            else
            ...
            on E: Exception do
            begin
              lHTMLOut.AppendFormat('<pre>Invalid metadata information: %s', [E.Message]);
            end;
          end;
        end
        else
      ...
      begin
        lHTMLOut.AppendFormat('<h2>No image availables in the last %d minutes</h2>',
          [MONITORED_MINUTES]);
        Break;
      end;
    end;
...
procedure TwmMain.wmMainwaPhotoAction(Sender: TObject; Request: TWebRequest; Response: TWebResponse;
var Handled: Boolean);
var
  ...
  begin
    ...
    if TryStrToFloat(Request.QueryFields.Values['lat'], lLat, FFormatSettings) then
      lInfoObject.AddPair('lat', TJSONNumber.Create(lLat));
    if TryStrToFloat(Request.QueryFields.Values['lon'], lLon, FFormatSettings) then
      lInfoObject.AddPair('lon', TJSONNumber.Create(lLon));
    TFile.WriteAllText('images' + PathDelim + lFileName + '.info', lInfoObject.ToString);
  end;
